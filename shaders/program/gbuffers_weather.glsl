...
void main() {
    ...
    #ifdef END
        // Bliss-style End weather: particles lit by End atmospheric ambient only.
        // No directional sun in the End — use pure ambient + blocklight.
        float albedoLuma = dot(sqrt3(color.rgb), vec3(0.299, 0.587, 0.114));
        color.rgb = vec3(albedoLuma) * ambientColor * 5.0 * lmCoord.y
                  + sqrt3(color.rgb) * blocklightCol * 2.0 * lmCoord.x;
    #else
